...
namespace Rainbow.Helpers
{
	...
	public class SearchHelper
	{
		...
		public static void AddToDropDownList(int portalID, ref DropDownList ddList)
		{
			...
			using (SqlDataReader drModules = GetSearchableModules(portalID)) 
			{
                ddList.Items.Add(new ListItem(Esperantus.Localize.GetString("ALL", "All", null), string.Empty));
		...
		{ 
			...
			string [] asRoles=Rainbow.Security.PortalSecurity.GetRoles(); 
...
*/
			...
			{ 
				...
				foreach (string sRole in asRoles) 
					select.Append(" OR (mod.AuthorizedViewRoles LIKE '%" + sRole + "%')"); 
				select.Append(")"); 
				select.Append(" AND ((tab.AuthorizedRoles LIKE '%All Users%')"); 
				if (context.Request.IsAuthenticated) // - no tenia en cuenta el rol "Authenticated users" 
					select.Append(" OR (tab.AuthorizedRoles LIKE '%Authenticated Users%')"); 
				foreach (string sRole in asRoles) 
					select.Append(" OR (tab.AuthorizedRoles LIKE '%" + sRole + "%')"); 
				select.Append(")"); 
		...
		public static string[] GetTopicList(int portalID)
		{
            ...
            al.Add(Esperantus.Localize.GetString("PORTALSEARCH_ALL", "All", null));
