...
namespace Rainbow.UI.WebControls
{
	...
	public class PagingNumbers : WebControl, INamingContainer, IPaging 
	{
		...
		int m_pageNumber = 0;
		int totalPages = 0;
		const int defaultRecordCount = 0;
		const bool isPagingEnabled = false;
		int pageSize = -1;

		// Controls
		System.Web.UI.WebControls.Label currentPage;
		PlaceHolder previousButton;
		PlaceHolder nextButton;
		LinkButton prev;
		LinkButton next;
		System.Web.UI.WebControls.LinkButton[] numericalLinkButtons;
...
//				{
//					pageNumber = (Convert.ToInt32(Context.Request.QueryString["PageIndex"]) - 1);
//				}
//
		}
		...
		{
            currentPage.Text = Esperantus.Localize.GetString("PAGE", "Page", null) + " " + (PageIndex + 1).ToString("n0") + " " + Esperantus.Localize.GetString("OF", "of", null) + " " + TotalPages.ToString("n0");
		}
