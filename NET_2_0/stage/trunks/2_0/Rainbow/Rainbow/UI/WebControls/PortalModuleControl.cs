...
namespace Rainbow.UI.WebControls
{
	...
	public class PortalModuleControl : UserControl, ISearchable, IInstaller
	{
		...
		public PortalModuleControl()
		{
            ...
            themeOptions.Add(new SettingOption((int)ThemeList.Default, Esperantus.Localize.GetString("MODULESETTINGS_THEME_DEFAULT", "Default", null)));
            themeOptions.Add(new SettingOption((int)ThemeList.Alt, Esperantus.Localize.GetString("MODULESETTINGS_THEME_ALT", "Alt", null)));
			...
			if (HttpContext.Current != null) // null in DesignMode
			{
				...
				if (_portalSettings != null)
				{
					...
					if (_portalSettings.CustomSettings.ContainsKey("SITESETTINGS_ALLOW_MODULE_CUSTOM_THEMES") && _portalSettings.CustomSettings["SITESETTINGS_ALLOW_MODULE_CUSTOM_THEMES"].ToString().Length != 0 && bool.Parse(_portalSettings.CustomSettings["SITESETTINGS_ALLOW_MODULE_CUSTOM_THEMES"].ToString()))
					{
            ...
            buttonDisplayOptions.Add(new SettingOption((int)ModuleButton.RenderOptions.ImageOnly, Esperantus.Localize.GetString("MODULESETTINGS_BUTTON_DISPLAY_IMAGE", "Image only", null)));
            buttonDisplayOptions.Add(new SettingOption((int)ModuleButton.RenderOptions.TextOnly, Esperantus.Localize.GetString("MODULESETTINGS_BUTTON_DISPLAY_TEXT", "Text only", null)));
            buttonDisplayOptions.Add(new SettingOption((int)ModuleButton.RenderOptions.ImageAndTextCSS, Esperantus.Localize.GetString("MODULESETTINGS_BUTTON_DISPLAY_BOTH", "Image and Text (CSS)", null)));
            buttonDisplayOptions.Add(new SettingOption((int)ModuleButton.RenderOptions.ImageOnlyCSS, Esperantus.Localize.GetString("MODULESETTINGS_BUTTON_DISPLAY_IMAGECSS", "Image only (CSS)", null)));
		...
		protected override void OnLoad(EventArgs e)
		{
			...
			if (version != null)
			{
				WorkFlowVersion requestedVersion = version == "Staging" ? WorkFlowVersion.Staging : WorkFlowVersion.Production;
...
*/
			...
			// jes1111 - if (GlobalResources.SupportWindowMgmt && this.SupportCollapsable)
		...
		protected override void OnInit(EventArgs e)
		{
			...
			if (DeleteBtn != null)
			{
				...
				else
				{
					...
					DeleteBtn.Click += new ImageClickEventHandler(this.DeleteBtn_Click);
                    DeleteBtn.AlternateText = Esperantus.Localize.GetString("DELETE");
			...
			if (EditBtn != null)
			{
				...
				else
				{
					...
					EditBtn.Click += new ImageClickEventHandler(this.EditBtn_Click);
                    EditBtn.AlternateText = Esperantus.Localize.GetString("Edit");
			...
			if (this.updateButton != null)
			{
                ...
                updateButton.Text = Esperantus.Localize.GetString("UPDATE");
		...
		public bool SupportsHelp
		{
			get
			{
				if ((Settings["MODULESETTINGS_SHOW_HELP_BUTTON"] == null || bool.Parse(Settings["MODULESETTINGS_SHOW_HELP_BUTTON"].ToString())) && (ModuleConfiguration.DesktopSrc.Length != 0))
				{
					string aux = Rainbow.Settings.Path.ApplicationRoot + "/rb_documentation/Rainbow/" + ModuleConfiguration.DesktopSrc.Replace(".", "_").ToString();
		...
		public ModuleButton.RenderOptions ButtonsRenderAs
		{
			get
			{
				if (HttpContext.Current != null) // if it is not design time
				{
					if (this.Settings["MODULESETTINGS_BUTTON_DISPLAY"] != null && this.Settings["MODULESETTINGS_BUTTON_DISPLAY"].ToString().Length != 0)
						buttonsRenderAs = (ModuleButton.RenderOptions) int.Parse(this.Settings["MODULESETTINGS_BUTTON_DISPLAY"].ToString());
				}
			...
			get
			{
				if (propertiesButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanProperties)
					{
						...
						propertiesButton.Image = this.CurrentTheme.GetImage("Buttons_Properties", "Properties.gif");
						if (this.PropertiesUrl.IndexOf("?") >= 0) //Do not change if  the querystring is present (shortcut patch)
							...
							propertiesButton.HRef = this.PropertiesUrl;
						else
							propertiesButton.HRef = HttpUrlBuilder.BuildUrl(this.PropertiesUrl, this.PageID, "mID=" + this.ModuleID.ToString());
						propertiesButton.Target = this.PropertiesTarget;
						propertiesButton.RenderAs = this.ButtonsRenderAs;
					}
				}
			...
			get
			{
				if (securityButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanSecurity)
					{
						...
						securityButton.Image = this.CurrentTheme.GetImage("Buttons_Security", "Security.gif");
						if (this.SecurityUrl.IndexOf("?") >= 0) //Do not change if  the querystring is present (shortcut patch)
							securityButton.HRef = this.SecurityUrl;
						else
							securityButton.HRef = HttpUrlBuilder.BuildUrl(this.SecurityUrl, this.PageID, "mID=" + this.ModuleID.ToString());
						securityButton.Target = this.SecurityTarget;
						securityButton.RenderAs = this.ButtonsRenderAs;
					}
				}
			...
			get
			{
				if (deleteModuleButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanDeleteModule)
					{
						...
						deleteModuleButton.Image = this.CurrentTheme.GetImage("Buttons_DeleteModule", "Delete.gif");
						deleteModuleButton.RenderAs = this.ButtonsRenderAs;
						...
						if (this.Page.Request.Browser.JavaScript == true && !(this.Page.IsClientScriptBlockRegistered("confirmDelete")))
						{
							string[] s = {"CONFIRM_DELETE"};
							this.Page.RegisterClientScriptBlock("confirmDelete", PortalSettings.GetStringResource("Rainbow.aspnet_client.Rainbow_scripts.confirmDelete.js", s));
						}
						if (deleteModuleButton.Attributes["onclick"] != null)
							deleteModuleButton.Attributes["onclick"] = "return confirmDelete();" + deleteModuleButton.Attributes["onclick"];
						else
		...
		public ModuleButton HelpButton
		{
			get
			{
				if (helpButton == null && HttpContext.Current != null)
				{
					...
					{
						// build the HRef
						string aux = this.ModuleConfiguration.DesktopSrc.Replace(".", "_").ToString();
						...
						helpButton.PopUpOptions = "toolbar=1,location=0,directories=0,status=0,menubar=1,scrollbars=1,resizable=1,width=600,height=400,screenX=15,screenY=15,top=15,left=15";
						helpButton.Image = CurrentTheme.GetImage("Buttons_Help", "Help.gif");
						helpButton.RenderAs = this.ButtonsRenderAs;
					}
				}
				return helpButton;
			}
			...
			get
			{
				if (upButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanArrows && this.ModuleConfiguration.ModuleOrder != 1)
					{
						...
						upButton.Image = this.CurrentTheme.GetImage("Buttons_Up", "Up.gif");
						upButton.Attributes.Add("direction", "up");
						upButton.Attributes.Add("pane", this.ModuleConfiguration.PaneName.ToLower());
						upButton.RenderAs = this.ButtonsRenderAs;
		...
		public ModuleButton DownButton
		{
			get
			{
				if (downButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanArrows)
					{
						ArrayList sourceList = GetModules(this.ModuleConfiguration.PaneName.ToLower());
						...
						if (this.ModuleConfiguration.ModuleOrder != m.Order)
						{
							...
							downButton.Image = this.CurrentTheme.GetImage("Buttons_Down", "Down.gif");
							downButton.Attributes.Add("direction", "down");
							downButton.Attributes.Add("pane", this.ModuleConfiguration.PaneName.ToLower());
							downButton.RenderAs = this.ButtonsRenderAs;
			...
			get
			{
				if (leftButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanArrows && this.ModuleConfiguration.PaneName.ToLower() != "leftpane")
					{
						...
						leftButton.Image = this.CurrentTheme.GetImage("Buttons_Left", "Left.gif");
						leftButton.Attributes.Add("sourcepane", this.ModuleConfiguration.PaneName.ToLower());
						leftButton.Attributes.Add("targetpane", leftButtonTargetPane);
						leftButton.RenderAs = this.ButtonsRenderAs;
			...
			get
			{
				if (rightButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanArrows && this.ModuleConfiguration.PaneName.ToLower() != "rightpane")
					{
						...
						rightButton.Image = this.CurrentTheme.GetImage("Buttons_Right", "Right.gif");
						rightButton.Attributes.Add("sourcepane", this.ModuleConfiguration.PaneName.ToLower());
						rightButton.Attributes.Add("targetpane", rightButtonTargetPane);
						rightButton.RenderAs = this.ButtonsRenderAs;
			...
			get
			{
				if (readyToApproveButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanRequestApproval)
					{
						...
						readyToApproveButton.TranslationKey = this.ReadyToApproveText;
						readyToApproveButton.EnglishName = "Request approval";
						readyToApproveButton.HRef = HttpUrlBuilder.BuildUrl("~/DesktopModules/Workflow/RequestModuleContentApproval.aspx", this.PageID, "mID=" + this.ModuleID.ToString());
						readyToApproveButton.Image = this.CurrentTheme.GetImage("Buttons_ReadyToApprove", "ReadyToApprove.gif");
						readyToApproveButton.RenderAs = this.ButtonsRenderAs;
					}
				}
			...
			get
			{
				if (revertButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanRequestApproval)
					{
						...
						revertButton.TranslationKey = this.RevertText;
						revertButton.EnglishName = "Revert";
						revertButton.Image = this.CurrentTheme.GetImage("Buttons_Revert", "Revert.gif");
						revertButton.ServerClick += new EventHandler(RevertToProductionContent);
						revertButton.RenderAs = this.ButtonsRenderAs;
					}
				}
				return revertButton;
			}
			...
			get
			{
				if (approveButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanApproveReject)
					{
						...
						approveButton.TranslationKey = this.ApproveText;
						approveButton.EnglishName = "Approve";
						approveButton.HRef = HttpUrlBuilder.BuildUrl("~/DesktopModules/Workflow/ApproveModuleContent.aspx", this.PageID, "mID=" + this.ModuleID.ToString());
						approveButton.Image = this.CurrentTheme.GetImage("Buttons_Approve", "Approve.gif");
						approveButton.RenderAs = this.ButtonsRenderAs;
					}
				}
			...
			get
			{
				if (rejectButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanApproveReject)
					{
						...
						rejectButton.TranslationKey = this.RejectText;
						rejectButton.EnglishName = "Reject";
						rejectButton.HRef = HttpUrlBuilder.BuildUrl("~/DesktopModules/Workflow/RejectModuleContent.aspx", PageID, "mID=" + this.ModuleID.ToString());
						rejectButton.Image = this.CurrentTheme.GetImage("Buttons_Reject", "Reject.gif");
						rejectButton.RenderAs = this.ButtonsRenderAs;
					}
				}
				return rejectButton;
			}
			...
			get
			{
				if (publishButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanPublish)
					{
						...
						publishButton.TranslationKey = this.PublishText;
						...
						// end of modification
						publishButton.Image = this.CurrentTheme.GetImage("Buttons_Publish", "Publish.gif");
						publishButton.RenderAs = this.ButtonsRenderAs;
					}
				}
			...
			get
			{
				if (_versionButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanVersion)
					{
						...
						{
							_versionButton.TranslationKey = this.ProductionVersionText;
							_versionButton.EnglishName = "To production version";
							_versionButton.Image = CurrentTheme.GetImage("Buttons_VersionToProduction", "VersionToProduction.gif");
						}
						else
						{
							_versionButton.TranslationKey = this.StagingVersionText;
							_versionButton.EnglishName = "To staging version";
							_versionButton.Image = CurrentTheme.GetImage("Buttons_VersionToStaging", "VersionToStaging.gif");
						}
						_versionButton.HRef = GetOtherVersionUrl();
						_versionButton.Target = this.EditTarget;
						_versionButton.RenderAs = this.ButtonsRenderAs;
					}
				}
			...
			get
			{
				if (editButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanEdit)
					{
						...
						editButton.TranslationKey = this.EditText;
						editButton.EnglishName = "Edit";
						if (this.EditUrl.IndexOf("?") >= 0) //Do not change if  the querystring is present
							//if ( this.ModuleID != this.OriginalModuleID )
							this.editButton.HRef = this.EditUrl;
						else
							this.editButton.HRef = HttpUrlBuilder.BuildUrl(this.EditUrl, this.PageID, "mID=" + this.ModuleID.ToString());
						editButton.Target = this.EditTarget;
						editButton.Image = this.CurrentTheme.GetImage("Buttons_Edit", "Edit.gif");
						editButton.RenderAs = this.ButtonsRenderAs;
					}
				}
				return editButton;
			}
			...
			get
			{
				if (addButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanAdd)
					{
						...
						addButton.TranslationKey = this.AddText;
						addButton.EnglishName = "Add";
						if (this.AddUrl.IndexOf("?") >= 0) //Do not change if  the querystring is present
							//if ( this.ModuleID != this.OriginalModuleID )
							this.AddButton.HRef = this.AddUrl;
						else
							this.AddButton.HRef = HttpUrlBuilder.BuildUrl(this.AddUrl, this.PageID, "mID=" + this.ModuleID.ToString());
						addButton.Target = this.AddTarget;
						addButton.Image = this.CurrentTheme.GetImage("Buttons_Add", "Add.gif");
						addButton.RenderAs = this.ButtonsRenderAs;
					}
				}
				return addButton;
			}
			...
			get
			{
				if (backButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanBack)
					{
						...
						backButton.HRef = this.Request.UrlReferrer.ToString();
						backButton.Image = this.CurrentTheme.GetImage("Buttons_Back", "Back.gif");
						backButton.RenderAs = this.ButtonsRenderAs;
					}
				}
				return backButton;
			}
			...
			get
			{
				if (printButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanPrint)
					{
						...
						_url.Append(Rainbow.Settings.Path.ApplicationRoot);
						...
						printButton.Image = this.CurrentTheme.GetImage("Buttons_Print", "Print.gif");
						...
						printButton.PopUpOptions = "toolbar=1,menubar=1,resizable=1,scrollbars=1,width=600,height=400,left=15,top=15";
						printButton.RenderAs = this.ButtonsRenderAs;
					}
				}
				return printButton;
			}
			...
			get
			{
				if (emailButton == null && HttpContext.Current != null)
				{
					...
					if (this.CanEmail)
					{
						...
						//					EmailButton.Text = Esperantus.Localize.GetString("BTN_Email","Email this",null) + "...";
		...
		public LinkButton MinMaxButton
		{
			get
			{
				...
				{
					// check authority
					if (this._vcm != null && !UserDesktop.isClosed(_vcm.ModuleID) && this.CanMinimized)
					{
						// create the button based on current view
						if (!UserDesktop.isMinimized(_vcm.ModuleID))
							minMaxButton = _vcm.create(WindowStateStrings.ButtonMinName, WindowStateStrings.ButtonMinLocalized);
						else
						{
							minMaxButton = _vcm.create(WindowStateStrings.ButtonMaxName, WindowStateStrings.ButtonMaxLocalized);
		...
		public LinkButton CloseButton
		{
			get
			{
				...
				{
					// check authority
					// jes1111 - if (this._vcm != null && !UserDesktop.isClosed(_vcm.ModuleID) && this.CanClose && GlobalResources.SupportWindowMgmtClose)
					if (this._vcm != null && !UserDesktop.isClosed(_vcm.ModuleID) && this.CanClose && Config.WindowMgmtWantClose)
					{
						// create the button
						closeButton = _vcm.create(WindowStateStrings.ButtonCloseName, WindowStateStrings.ButtonClosedLocalized);
		...
		public bool CanDeleteModule
		{
			get
			{
				if (PortalSecurity.IsInRoles(this.ModuleConfiguration.AuthorizedDeleteModuleRoles) && this.portalSettings.ActivePage.PageID == ModuleConfiguration.PageID)
					return true;
				else
					return false;
			}
		...
		public bool CanEdit
		{
			get
			{
				if (this.ModuleConfiguration == null || this.portalSettings.ActivePage.PageID != ModuleConfiguration.PageID)
					return false;

				if ((this.SupportsWorkflow && this.Version == WorkFlowVersion.Staging) || !this.SupportsWorkflow)
				{
					if ((PortalSecurity.IsInRoles(this.ModuleConfiguration.AuthorizedEditRoles)) && (EditUrl != null) && (EditUrl.Length != 0) && (this.WorkflowStatus == WorkflowState.Original || this.WorkflowStatus == WorkflowState.Working))
					{
						return true;
					}
					else
		...
		public bool CanAdd
		{
			get
			{
				...
				if ((this.SupportsWorkflow && this.Version == WorkFlowVersion.Staging) || !this.SupportsWorkflow)
				{
					if ((PortalSecurity.IsInRoles(this.ModuleConfiguration.AuthorizedAddRoles)) && (AddUrl != null) && (AddUrl.Length != 0) && (this.WorkflowStatus == WorkflowState.Original || this.WorkflowStatus == WorkflowState.Working))
					{
						return true;
					}
					else
		...
		public bool CanArrows
		{
			get
			{
				...
				if (this.SupportsArrows && this.portalSettings.ActivePage.PageID == this.ModuleConfiguration.PageID && PortalSecurity.IsInRoles(this.ModuleConfiguration.AuthorizedMoveModuleRoles))
					return true;
				else
					return false;
			}
		...
		public bool CanSecurity
		{
			get
			{
				...
				if (PortalSecurity.IsInRoles(this.ModuleConfiguration.AuthorizedPropertiesRoles) && this.portalSettings.ActivePage.PageID == ModuleConfiguration.PageID && this.SecurityUrl != null && this.SecurityUrl.Length != 0)
					return true;
				else
					return false;
			}
		...
		public bool CanProperties
		{
			get
			{
				...
				if (PortalSecurity.IsInRoles(this.ModuleConfiguration.AuthorizedPropertiesRoles) && this.portalSettings.ActivePage.PageID == ModuleConfiguration.PageID && this.PropertiesUrl != null && this.PropertiesUrl.Length != 0)
					return true;
				else
					return false;
			}
		...
		private string GetOtherVersionUrl()
		{
			...
			qs.Add("wversion" + this.ModuleConfiguration.ModuleID.ToString() + "=" + (this.Version == WorkFlowVersion.Production ? WorkFlowVersion.Staging.ToString() : WorkFlowVersion.Production.ToString()));
		...
		public string GetLastModified()
		{
			...
			if (bool.Parse(((SettingItem) portalSettings.CustomSettings["SITESETTINGS_SHOW_MODIFIED_BY"]).Value) && bool.Parse(((SettingItem) Settings["MODULESETTINGS_SHOW_MODIFIED_BY"]).Value))
			{
                ...
                return "<span class=\"LastModified\">" + Esperantus.Localize.GetString("LMB_LAST_MODIFIED_BY", "Last modified by", null) + "&#160;" + Email + "&#160;" + Esperantus.Localize.GetString("LMB_ON", "on", null) + "&#160;" + TimeStamp.ToLongDateString() + " " + TimeStamp.ToShortTimeString() + "</span>";
			}
			else
			{
				return string.Empty;
		...
		private ArrayList GetModules(string pane)
		{
			...
			foreach (ModuleSettings _module in this.Page.portalSettings.ActivePage.Modules)
			{
				if (this.portalSettings.ActivePage.PageID == _module.PageID && _module.PaneName.ToLower() == pane.ToLower())
				{
		...
		private void RightLeft_Click(Object sender, EventArgs e)
		{
			...
			this.Page.portalSettings = (PortalSettings) Context.Items["PortalSettings"];
			...
			// Redirect to the same page to pick up changes
			Page.Response.Redirect(AppendModuleID(Page.Request.RawUrl, this.ModuleID));
		}
		...
		protected virtual void BuildControlHierarchy()
		{
			...
			this._header.Controls.AddAt(0, new LiteralControl(string.Format("<div id=\"mID{0}\" class=\"{1} ModuleWrap\">", this.ModuleID.ToString(), this.GetType().Name)));
		...
		protected virtual void Build()
		{
			...
			else
			{
				...
				this._footer.Controls.Add(CurrentTheme.GetLiteralControl("ControlTitleAfterControl"));
