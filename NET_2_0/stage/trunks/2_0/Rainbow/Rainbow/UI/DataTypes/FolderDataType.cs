	...
	public class FolderDataType : BaseDataType
	{
		...
		public FolderDataType(string baseDirectory, string defaultDirectory)
		{
			...
			if (defaultDirectory != null) 
			{
				...
				{
					throw new ArgumentException("Cannot create the default directory '" + defaultDirectory + "'", "FolderDataType", ex);
				}
			}
		...
		protected override void InitializeComponents()
		{
			using (Panel panel = new Panel())
			{
				...
				using (TextBox tb = new TextBox())
				{
                    ...
                    tb.Text = Esperantus.Localize.GetString("NEW_FOLDER", "New Folder ?");
					tb.Columns = 30;
				...
				if (value is Panel)
				{
					...
					foreach (Control c in value.Controls) 
					{
						...
						if (c is TextBox) 
						{
                            ...
                            if (tb.Text != Esperantus.Localize.GetString("NEW_FOLDER", "New Folder ?")) 
							{
								try 
								{
					...
					throw new ArgumentException("A Panel values is required, a '" + value.GetType().Name + "' is given.", "EditControl");
			}
		}
	}
}