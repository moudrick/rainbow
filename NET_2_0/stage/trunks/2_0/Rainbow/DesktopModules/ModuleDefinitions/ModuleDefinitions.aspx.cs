...
namespace Rainbow.AdminAll 
{
	...
	/// </summary>
	public class ModuleDefinitions : EditItemPage
	{
		...
		protected Button btnUseInstaller;
		protected Button btnDescription;
		protected HtmlTable tableInstaller;
		protected HtmlTable tableManual;
			...
			if (Page.IsValid) 
			{
				try
				{
					if (!btnUseInstaller.Visible)
						ModuleInstall.InstallGroup(Server.MapPath(Path.ApplicationRoot + "/" + InstallerFileName.Text), lblGUID.Text == string.Empty);
					else
						ModuleInstall.Install(FriendlyName.Text, DesktopSrc.Text, MobileSrc.Text, lblGUID.Text == string.Empty);
				...
				catch(Exception ex)
				{
					...
					if (!btnUseInstaller.Visible)
						lblErrorDetail.Text += " Installer: " + Server.MapPath(Path.ApplicationRoot + "/" + InstallerFileName.Text);
					else
