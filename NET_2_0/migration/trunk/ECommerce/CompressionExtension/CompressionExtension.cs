...
namespace MasterCSharp.WebServices
{
	...
	public class CompressionExtension : System.Web.Services.Protocols.SoapExtension
	{
		...
		public override void ProcessMessage(SoapMessage message) 
		{
			...
			switch (message.Stage) 
			{
				...
				case SoapMessageStage.AfterSerialize:
					// ZIP the contents of the SOAP Body after serialization
					Zip();
					break;

				case SoapMessageStage.BeforeDeserialize:
					// Unzip the contents of the SOAP Body before serialization
					Unzip();
					break;
		...
		private byte[] Unzip(string stringToUnzip) 
		{
			...
			try
			{
				inputByteArray = Convert.FromBase64String(stringToUnzip);
			}
			catch(FormatException ex)
			{
				throw new Exception("Invalid base 64 string: '" + stringToUnzip + "'", ex);
			}
