...
namespace Rainbow.Helpers
{
	...
	/// </summary>
	[History("gman3001", "2004/10/26", "Added Method to retrieve a list of all AD Groups that a user account is a member of.")]
	public class ADHelper
	{
		...
		public static EmailAddressList GetEmailAddresses(string _account)
		{
			// Lookup the domain in which we must look for the user or group
			string[] account = _account.Split("\\".ToCharArray());

			if (account.Length != 2)
				return new EmailAddressList(); // not a valid windows account!
			DirectoryEntry rootEntry = null;
			string[] domains = ConfigurationSettings.AppSettings["ADdns"].Split(";".ToCharArray());
			...
			mySearcher.Filter = "(&(|(objectClass=group)(&(objectClass=user)(objectCategory=person)))(sAMAccountName=" + account[1] + "))";
			...
			{
				throw new ApplicationException("Could not get users/groups from domain '" + account[0] + "'.");
			}
					...
					using (DirectoryEntry rootEntry = GetDomainRoot(aDDomain))
					{
						if (aDDomain.Trim().ToLower().StartsWith("ldap://"))
						{
							...
							try
							{
								...
								foreach (SearchResult resEnt in mySearcherSearchResult)
								{
									...
									ADAccountType accounttype = GetAccountType(entry);
									dt.Rows.Add(new Object[] {name, DomainName + "\\" + abbreviation, accounttype.ToString()});
								}
							}

							catch
							{
								throw new ApplicationException("Could not get users/groups from domain '" + aDDomain + "'.");
							}
						}
						...
						else if (aDDomain.Trim().ToLower().StartsWith("winnt://"))
						{
							...
							foreach (DirectoryEntry user in rootEntry.Children)
							{
								...
								dt.Rows.Add(new Object[] {fullname, DomainName + "\\" + fullname, ADAccountType.user.ToString()});
							}
							// Get the users
							...
							foreach (DirectoryEntry user in rootEntry.Children)
							{
								...
								string accountname = user.Name;
								dt.Rows.Add(new Object[] {fullname, DomainName + "\\" + fullname, ADAccountType.group.ToString()});
							}
						}
					}
		...
		public static string[] GetUserGroups(string userAccount)
		{
			...
			if (userAccount != null)
			{
				...
				{
					throw new ApplicationException("Could not get users/groups from domain '" + account[0] + "'. Either the user/group does not exist, or you do not have the necessary permissions in the Active Directory.");
				}
				...
				if (accounttype == ADAccountType.user)
				{
					try
					{
						...
						{
							DirectoryEntry currentEntry = new DirectoryEntry(rootPath + entry.Properties["memberOf"][i].ToString());
		...
		private static DirectoryEntry GetDomainRoot(string domain)
		{
			...
			if (PortalSettings.EnableADUser)
				return new DirectoryEntry(domain, PortalSettings.ADUserName, PortalSettings.ADUserPassword);
		...
		///     A string value...
		/// </returns>
		private static string GetNetbiosName(string domain)
		{
			return GetNetbiosName(GetDomainRoot(domain));
		}
		...
		private static string GetNetbiosName(DirectoryEntry root)
		{
			...
			// 2004-07-28, Leo Duran, Fix for IIS not being run on the AD Computer
			if (PortalSettings.EnableADUser)
				entry = new DirectoryEntry("LDAP://" + path.Insert(path.IndexOf("/") + 1, "CN=Partitions, CN=Configuration,"), PortalSettings.ADUserName, PortalSettings.ADUserPassword);

			else
				entry = new DirectoryEntry("LDAP://" + path.Insert(path.IndexOf("/") + 1, "CN=Partitions, CN=Configuration,"));
			// End 2004-07-28, Leo Duran
			...
			{
				throw new ApplicationException("Domain could not be contacted.", ex);
			}
		}
