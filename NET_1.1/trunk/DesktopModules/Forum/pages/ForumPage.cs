...
namespace yaf.pages
{
	...
	public class ForumPage : System.Web.UI.UserControl
	{
		...
		private ObjectHandle hdlRainbowForum; //KAW
		#endregion
			...
			*/
			...
			switch(authType)
			{
				...
				case AuthType.Rainbow:
					typeUser = "Rainbow.DesktopModules.RainbowUser";
					break;
			...
			//KAW -- create a instance of the RainbowUser class in the ForumModule assembly
			hdlRainbowForum = Activator.CreateInstance("ForumModule",typeUser);
			m_forumUser = (IForumUser)hdlRainbowForum.Unwrap();
			...
			string sPassword = FormsAuthentication.HashPasswordForStoringInConfigFile(User.Password,"md5");//KAW
			object userID = DB.user_login(PageBoardID,User.Name,sPassword);//KAW
			
			
			if(m_pageinfo==null && authType!=AuthType.Forms && User.IsAuthenticated && userID==DBNull.Value) 
			{
				if(!DB.user_register(this,PageBoardID,User.Name,User.Password,User.Email,User.Location,User.HomePage,0,false))
					throw new ApplicationException("User registration failed.");
					
				if (User.IsRainbowAdmin) //KAW
				{
					object userID2 = DB.user_login(PageBoardID,User.Name,sPassword);//KAW
					DB.user_adminsave(PageBoardID,userID2,User.Name,User.Email,true,1);
				}
			...
			else
			{
				   ...
				   object userID2 = DB.user_login(PageBoardID,User.Name,sPassword);//KAW
				   DataTable dt = DB.user_list(PageBoardID,userID2,null);
				   DataRow row = dt.Rows[0];

				    //If User is admin make him Admin in Forum
				if (User.IsRainbowAdmin  && ((int)row["RankID"]) != 1) //KAW
				{
				         
					DB.user_adminsave(PageBoardID,userID2,User.Name,User.Email,true,1);

				}
				else if(!User.IsRainbowAdmin  && ((int)row["RankID"]) == 1) //KAW
				{
					DB.user_adminsave(PageBoardID,userID2,User.Name,User.Email,false,3);
		...
		protected override void Render(System.Web.UI.HtmlTextWriter writer) 
		{
			if(m_bShowToolBar) 
			{
				...
				{
					//KAW
					footer.AppendFormat("Forum Version {0}.",Data.AppVersionName);
					footer.AppendFormat("<br/>Copyright &copy; 2003-2004  All rights reserved.");
				}
				else 
				{
					//KAW
					footer.AppendFormat(GetText("COMMON","POWERED_BY"),
						String.Format("<a target=\"_top\" title=\"Rainbow Portal\" href=\"http://www.rainbowportal.net/\">Rainbow Portal</a>"),
...
#if DEBUG
				//KAW
				footer.AppendFormat("<br/>{0} queries ({1:N3} seconds, {2:N2}%).<br/>{3}",QueryCounter.Count,QueryCounter.Duration,100 * QueryCounter.Duration/hiTimer.Duration,QueryCounter.Commands.Replace("yaf_",""));
#endif
