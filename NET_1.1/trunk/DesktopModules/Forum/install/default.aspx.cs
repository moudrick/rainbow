...
namespace yaf.install
{
	...
	public class _default : System.Web.UI.Page
	{
		...
		protected yaf.controls.SmartScroller Smartscroller1;//KAW
	...
	public IForumUser USER //KAW
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!HttpContext.Current.User.IsInRole("Admins"))
				Response.Redirect("InstallError.aspx");

                       //KAW
			string[] AdminInfo = HttpContext.Current.User.Identity.Name.Split('|');
			this.UserName.Text = AdminInfo[0].ToString();
			this.AdminEmail.Text = AdminInfo[1].ToString();
...
//				}
				try 
				{
					string typeUser = "Rainbow.DesktopModules.RainbowUser";
					...
					using(SqlCommand cmd = new SqlCommand("yaf_system_initialize")) 
					{
						...
						cmd.Parameters.Add("@Password",FormsAuthentication.HashPasswordForStoringInConfigFile(USER.Password,"md5"));
						DB.ExecuteNonQuery(cmd);
					}
						...
						using(DataTable dt=new DataTable("sysobjects")) 
						{
							...
							using(SqlCommand cmd=conn.CreateCommand()) 
							{
								...
								if(bGrant) 
								{
									...
									{
										cmd.CommandText = string.Format("grant execute on {0} to {1}",row["Name"],"\"" + userName + "\"");//KAW
										cmd.ExecuteNonQuery();
									}
									foreach(DataRow row in dt.Select("IsUserTable=1 or IsView=1")) 
									{
										cmd.CommandText = string.Format("grant select,update on {0} to {1}",row["Name"],"\"" + userName + "\"");//KAW
										cmd.ExecuteNonQuery();
									}
								} 
								else 
