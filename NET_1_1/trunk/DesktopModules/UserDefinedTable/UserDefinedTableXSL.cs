...
namespace Rainbow.DesktopModules
{
	...
	public class UserDefinedTableXML : Xml, IPostBackEventHandler 
	{
		...
		#region Constructors
		public UserDefinedTableXML(DataView XMLdataview, int PageID, int ModuleID, bool IsEditable, string SortField, string SortOrder)
		{
			dv=XMLdataview;
			pageID=PageID;
			moduleID=ModuleID;
		...
		private string RenderedXML()
		{
			...
			while (cmdPos>-1)
			{
				...
				sb.Replace(srepl,Localize.GetString(lkey));
				cmdPos = sb.ToString().ToUpper().IndexOf("@@LOCALIZE");
			}
...
//			}
		...
		private XmlDocument XmlData()
		{
			...
			newAttribute.Value = Localize.GetCurrentNeutralCultureName();
			rootElement.Attributes.Append(newAttribute);
			newAttribute=null;
			...
			{      
				...
				if(showdetailID == 0  || showdetailID.ToString()==drv["UserDefinedRowID"].ToString()) //Don't bother to make xml data for ever record if we only want one 
				{
					...
					if(IsEditable)
					{
						extraNode.InnerText = HttpUrlBuilder.BuildUrl("~/DesktopModules/UserDefinedTable/UserDefinedTableEdit.aspx", pageID, "&mID=" + ModuleID + "&UserDefinedRowID=" + drv["UserDefinedRowID"].ToString()); 
					}		
					else
					{
		...
		private string GetSortOrderImg(string order)
		{
			...
			if(order=="DESC")
			{
				s = @"<img src='" + Path.WebPathCombine(Path.ApplicationRoot,"DesktopModules/UserDefinedTable/sortdescending.gif") + "' width='10' height='9' border='0'>";
			}
			else
			{
				s = @"<img src='" + Path.WebPathCombine(Path.ApplicationRoot,"DesktopModules/UserDefinedTable/sortascending.gif") + "' width='10' height='9' border='0'>";
			}
			return s;
		}
