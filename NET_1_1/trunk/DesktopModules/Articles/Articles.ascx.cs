	...
	public class Articles : PortalModuleControl
	{
		...
		public Articles()
		{
			...
			if(portalSettings != null) //check for avoid design time errors
			{
				...
				SettingItem RolesViewExpiredItems = new SettingItem(new CheckBoxListDataType(users.GetPortalRoles(portalSettings.PortalID), "RoleName", "RoleName"));
		...
		{
			SearchDefinition s = new SearchDefinition("rb_Articles", "Title", "Abstract", "CreatedByUser", "CreatedDate", searchField);
