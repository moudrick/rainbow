...
{
	public class AddPage : EditItemPage
	{
		...
		protected Literal lbl_jump_to_tab;
		protected CheckBox cb_JumpToPage;
		...
		{
			string returnPage = HttpUrlBuilder.BuildUrl("~/DesktopDefault.aspx", int.Parse(Request.QueryString["returntabid"]));
			...
			if (Page.IsValid == true) 
			{
				try
				{
					...
					if (cb_JumpToPage.Checked == true) 
					{
						...
						returnPage = HttpUrlBuilder.BuildUrl("~/DesktopDefault.aspx", NewPageID, "SelectedPageID=" + NewPageID.ToString());
					}
					else
					{
						...
						returnPage = HttpUrlBuilder.BuildUrl("~/DesktopModules/Pages/AddPage.aspx", "mID=" + Request.QueryString["mID"] + "&returntabid=" + Request.QueryString["returntabid"]);
					}
					Response.Redirect(returnPage);
		...
		private int SavePageData() 
		{
			...
			//jes111 - useMemberList |= ConfigurationSettings.AppSettings["LDAPLogin"] != null ? true : false;
			...
			int NewPageID = new PagesDB().AddPage(portalSettings.PortalID, Int32.Parse(parentPage.SelectedItem.Value), tabName.Text, 990000, authorizedRoles, showMobile.Checked, mobilePageName.Text);
