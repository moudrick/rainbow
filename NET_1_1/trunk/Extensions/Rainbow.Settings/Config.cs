			...
			get
			{
				...
				if (!CurrentCache.Exists(keyConnection)) // not in cache
				{
					if (EnableMultiDbSupport)
						...
						siteConnectionString = GetString(keyConnection, GetString("ConnectionString", "server=localhost;database=Rainbow;uid=sa;pwd=", false), false);
					else 
						// look in web.config for key="ConnectionString"
						siteConnectionString = GetString("ConnectionString", "server=localhost;database=Rainbow;uid=sa;pwd=", false);
					// add to cache
