		...
		public static string dbKey
		{
			get
			{
				...
				if (Config.EnableMultiDbSupport)
					dbKey = "DatabaseVersion_" + Config.SqlConnectionString.DataSource+"_"+Config.SqlConnectionString.Database; // For multidb support
				return dbKey;
			}
		}
		...
		public static int DatabaseVersion
		{
			...
			get
			{
				...
				if(HttpContext.Current.Application[dbKey] == null)
				{
					...
					{
						throw new DatabaseUnreachableException("Failed to get Database Version - most likely cannot connect to db or no permission.",ex); // Jes1111
						//Rainbow.Configuration.ErrorHandler.HandleException("If this fails most likely cannot connect to db or no permission", ex);
