	...
	public class ADHelper
	{
		...
		public static string[] GetUserGroups(string UserAccount)
		{
			...
			if (UserAccount != null)
			{
				...
				string[] domains = Config.ADdns.Split(";".ToCharArray()); // jes1111 - ConfigurationSettings.AppSettings["ADdns"].Split(";".ToCharArray());
				...
				{
					throw new Exception("Could not get users/groups from domain '" + account[0] + "'. Either the user/group does not exist, or you do not have the necessary permissions in the Active Directory.");
				}
		...
		private static string GetNetbiosName(DirectoryEntry root)
		{
			...
			if(Config.EnableADUser)
			{
				entry = new DirectoryEntry("LDAP://" + path.Insert(path.IndexOf("/")+1, "CN=Partitions, CN=Configuration,"), Config.ADUserName, Config.ADUserPassword);
			}
			else
			{
