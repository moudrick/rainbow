...
namespace Rainbow.Configuration
{
	...
	public class TabSettings
	{
		...
		[Obsolete("Please use PageSettings")]
		public static PagesBox GetTabSettingsTabsBox(int PageID)
		{
			// Create Instance of Connection and Command Object
			using (SqlConnection myConnection = Config.SqlConnectionString)
			{
					...
					using (SqlDataReader result = myCommand.ExecuteReader(CommandBehavior.CloseConnection))
					{
						...
						try
						{
							while (result.Read())
							{
								...
								Hashtable cts = new TabSettings().GetTabCustomSettings(tabDetails.PageID);
