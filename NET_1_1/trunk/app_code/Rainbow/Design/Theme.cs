...
namespace Rainbow.Design
{
	...
	public class Theme
	{
		...
		public string GetModuleImageSRC(string image_file_name)
		{
			...
			{
				imagePath = Settings.Path.WebPathCombine(DefaultModuleImagePath.Replace("~", Settings.Path.ApplicationRoot), image_file_name);
			}
			...
			{
				imagePath = Settings.Path.WebPathCombine(DefaultModuleImagePath.Replace("~", Settings.Path.ApplicationRoot), "1x1.gif");				
			}
		...
		public string GetThemePart(string name)
		{
			...
			{
				ThemePart part = (ThemePart)this.ThemeParts[name];
				return part.Html;
			}
		...
		public string CssFile
		{
			get
			{
				return Settings.Path.WebPathCombine(WebPath, _Css);
			}
		}
		...
		public string Module_CssFile(string cssfilename)
		{
				...
				{
					cssfilPath = Settings.Path.WebPathCombine(DefaultModuleCSSPath.Replace("~", Settings.Path.ApplicationRoot), cssfilename);
				}
