...
namespace Rainbow.Services
{
    ...
    public class ServiceHelper
	{
		...
		static public bool FillRSSServiceRequestInfo(NameValueCollection Params, ref string parameterError, ref ServiceRequestInfo requestInfo)
		{
			bool retValue = true;
			string par;
		...
		static public ArrayList GetResponseTabList(int portalID, int userID, string applicationFullPath, ref ServiceRequestInfo requestInfo) 
		{
			...
			select.Append(" FROM rb_Tabs Tab, rb_Tabs ParTab, rb_Modules Mod, rb_ModuleDefinitions ModDef, rb_GeneralModuleDefinitions GenModDef");
			select.Append(" WHERE");
			select.Append(" Tab.AuthorizedRoles like '%All Users%'");
			select.Append(" AND Tab.ParentPageID *= ParTab.PageID");   // Jakob: I just luv this outer join! (It's so easy!)
			...
			select.Append(" ORDER BY Tab.PageOrder " + requestInfo.SortDirection + ", Tab.ParentPageID, Mod.ModuleOrder ");
				...
				while (dr.Read()) 
				{
					...
					if (PageID != PageIDPrev)
					{
						...
						if (requestInfo.ShowID)
							responseItem.Title = (string)dr["TabName"] + " (PageID=" + PageID + ")";
						else
							responseItem.Title = (string)dr["TabName"];
		...
		/// <param name="requestInfo">a filled ServiceRequestInfo class</param>
		/// <returns>ArrayList</returns>
		static public ArrayList GetResponseModuleList(int portalID, int userID, string applicationFullPath, ref ServiceRequestInfo requestInfo) 
		{
			ArrayList colItems = new ArrayList();
			ServiceResponseInfoItem responseItem = null;
			...
			select.Append(" FROM rb_Modules Mod, rb_Tabs Tab, rb_ModuleDefinitions ModDef, rb_GeneralModuleDefinitions GenModDef");
			select.Append(" WHERE");
			select.Append(" Tab.AuthorizedRoles like '%All Users%'");
			select.Append(" AND Tab.PageID = Mod.PageID");
			select.Append(" AND Mod.ModuleDefID = ModDef.ModuleDefID");
			...
			try
			{
				while (dr.Read()) 
				{
					...
					if (requestInfo.ShowID)
					{
						...
						responseItem.Description = "Type: " + (string)dr["FriendlyName"] + ", Page: " + (string)dr["TabName"] + " (PageID=" + PageID + ")";
					}
					else
					{
						responseItem.Title = (string)dr["ModuleTitle"] + " (ModuleID=" + ModuleID + ")";
						responseItem.Description = "Type: " + (string)dr["FriendlyName"] + ", Page: " + (string)dr["TabName"];
					}
					responseItem.Link = applicationFullPath + "/DesktopDefault.aspx?tabID=" + PageID.ToString();
					if (requestInfo.PortalAlias.Length != 0)
						responseItem.Link += "&Alias=" + requestInfo.PortalAlias; 
		...
		/// <param name="requestInfo">a filled ServiceRequestInfo class</param>
		/// <returns>ArrayList</returns>
		static public ArrayList GetResponseItemList(int portalID, int userID, string applicationFullPath, ref ServiceRequestInfo requestInfo)
		{
			ArrayList colItems = new ArrayList();
			...
			SqlDataReader dr;
			dr = SearchHelper.SearchPortal(portalID, userID, moduleType, requestInfo.SearchString, requestInfo.SearchField, requestInfo.SortField, requestInfo.SortDirection, string.Empty, select.ToString());
			...
			try
			{
				while (dr.Read() && hits<=requestInfo.MaxHits) 
				{
					...
					if (!dr.IsDBNull(7))
					{
						...
						if (requestInfo.ShowID) 
							responseItem.Description += " (PageID=" + responseItem.PageID + ", ModuleID=" +responseItem.ModuleID + ")";
					}
					if (!dr.IsDBNull(8))
...
//			else
//			{
				SqlDataReader dr = DBHelper.GetDataReader("SELECT PortalID FROM rb_Portals WHERE PortalAlias='" + portalAlias + "'");
...
//			}
        ...
        public static ServiceResponseInfo CallService(int portalID, int userID, string applicationFullPath, ref ServiceRequestInfo requestInfo, Page rainbowPage)
		{
			...
			if (requestInfo.Type != ServiceType.RSSService)
			{
				...
				if (requestInfo.LocalMode)
				{
					if (rainbowPage != null)
					{
						...
						responseInfo.ServiceImageUrl = Path.ApplicationFullPath + rainbowPage.portalSettings.PortalPath + "/logo.gif";
				...
				switch (requestInfo.Type)
				{
					case ServiceType.CommunityWebService:
						if (requestInfo.LocalMode)
						{
							...
							responseInfo.Items = GetResponseItemList(portalID, userID, applicationFullPath, ref requestInfo);
						...
						if (requestInfo.LocalMode)
						{
							switch (requestInfo.ListType)
							{
								case ServiceListType.Tab:
									responseInfo.Items = ServiceHelper.GetResponseTabList(portalID, userID, Path.ApplicationFullPath, ref requestInfo);
									break;
								case ServiceListType.Module:
									responseInfo.Items = ServiceHelper.GetResponseModuleList(portalID, userID, Path.ApplicationFullPath, ref requestInfo);
									break;
								case ServiceListType.Item:
									responseInfo.Items = ServiceHelper.GetResponseItemList(portalID, userID, Path.ApplicationFullPath, ref requestInfo);
									break;
							}
							if (responseInfo.ServiceStatus == string.Empty)
								responseInfo.ServiceStatus = "OK";
