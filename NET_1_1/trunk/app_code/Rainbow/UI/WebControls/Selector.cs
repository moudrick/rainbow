...
namespace Rainbow.UI.WebControls
{
	...
	{ 
		public Selector ()
		{
			...
			else this.LanguageListString = "en-US";
		}
		...
		protected override void OnChangeLanguage(LanguageSwitcherEventArgs e)
		{
			if (Context != null)
			{
				...
				string returnUrl = HttpUrlBuilder.BuildUrl (auxUrl, tID, mID, e.CultureItem.Culture, customParams, string.Empty, string.Empty);
				if (returnUrl.ToLower().IndexOf("lang") == -1) 
				{
					customParams += "&Lang=" +  e.CultureItem.Culture.Name;
					returnUrl = HttpUrlBuilder.BuildUrl (auxUrl, tID, mID, e.CultureItem.Culture, customParams, string.Empty, string.Empty);
				}
