...
//                           <Max: 1.3.0.0>             //
// <Script Version: 1300>   				//
// Additions by groskrg@versifit.com 7/28/2004	        //
//------------------------------------------------------//
var m_oSolpartMenu;
...
SolpartMenu.prototype.GetMenuItems = function (oParent)
{
  ...
  __db(sID + ' getmenuitems');
		  ...
		  if (sParentID == "-1")	//'if top level menu item
		  {
  			...
  			{
					//GG:Line Changed on 7/28/2004 by groskrg@versifit.com to support custom highlight css class and also replaces default item css class with the custom css class for this menu item if it exists.
					sHTML += '<td>\n<table width="100%" CELLPADDING="0" CELLSPACING="0">\n<tr id="td' + this._m_sNSpace + sID + '" ' + spm_getMenuBarEvents(this._m_sNSpace) + '  class="' + spm_fixCSSForMac(this.getIntCSSName('spmbar spmitm') + ' ' +this.cssMenuBar + ' ' + ((spm_getMenuItemCSS(oNode) == '')?this.cssMenuItem:spm_getMenuItemCSS(oNode))) + '" savecss="' + spm_getMenuItemCSS(oNode) + '" hlcss="'+spm_getMenuItemHLCSS(oNode)+'" menuclick="' + sClickAction + '" style="' + spm_getMenuItemStyle('item', oNode) + '">\n';
		  ...
		  else                        //'submenu - not top level menu item
		  {
			  switch(oNode.nodeName)
			  {
				  case "menuitem":
				  {
					//GG:Changed on 7/28/2004 by groskrg@versifit.com to support custom highlight css class and also replaces default item	css class with the custom css class for this menu item if it exists.
					  sHTML +=		'   <tr ID="tr' + this._m_sNSpace + sID + '" ' + spm_getMenuItemEvents(this._m_sNSpace) + ' parentID="' + sParentID + '" class="' + spm_fixCSSForMac(this.getIntCSSName('spmitm') + ' ' +((spm_getMenuItemCSS(oNode) == '')?this.cssMenuItem:spm_getMenuItemCSS(oNode))) + '" savecss="' + spm_getMenuItemCSS(oNode) + '" hlcss="'+spm_getMenuItemHLCSS(oNode)+'" menuclick="' + sClickAction + '" style="' + spm_getMenuItemStyle('item', oNode) + '">\n';
					  ...
					  sHTML +=		'       <td id="td' + this._m_sNSpace + sID + '" class="' + spm_fixCSSForMac(this.getIntCSSName('spmitm') + ' ' + ((spm_getMenuItemCSS(oNode) == '')?this.cssMenuItem:spm_getMenuItemCSS(oNode))) + '" savecss="' + spm_getMenuItemCSS(oNode) + '" hlcss="'+spm_getMenuItemHLCSS(oNode)+'" NOWRAP="NOWRAP" TITLE="' + spm_getAttr(oNode, 'tooltip', '') + '" style="' + spm_getMenuItemStyle('item', oNode) + '">' + spm_getItemHTML(oNode, 'left', '') + spm_getAttr(oNode, 'title', '') + spm_getItemHTML(oNode, 'right', '') + '</td>\n';
					  //GG:End change
  ...
  //--- menubar mouseover event ---//
	SolpartMenu.prototype.onMBMO = function (e)
	{
		...
		//GG:Changed on 7/28/2004 by groskrg@versifit.com to support custom highlight css class for this menu item if exists
		oCell.className =  spm_fixCSSForMac(this.getIntCSSName('spmitmsel spmbar') + this.cssMenuBar + ' ' + ((spm_getAttr(e, 'hlcss', '') == '')?this.cssMenuItemSel:spm_getAttr(e, 'hlcss', '')));
  ...
  //--- menubar mouseout event ---//
	SolpartMenu.prototype.onMBMOUT = function (e)
	{
		...
		//GG:Changed on 4/8/2004 by groskrg@versifit.com to support custom css class by replacing default item	css class with it if exists.	  	  
		oCell.className = spm_fixCSSForMac(this.getIntCSSName('spmbar spmitm') + ' '+ this.cssMenuBar + ' ' + ((spm_getAttr(e, 'savecss', '') == '')?this.cssMenuItem:spm_getAttr(e, 'savecss', '')));	  
    		this.stopTransition();
...
*/
  ...
  //--- handles mouseout for menu item ---//
	SolpartMenu.prototype.handlembi_mout = function (oRow)
	{
		  ...
		  //GG:Begin change on 7/28/2004 by groskrg@versifit.com to support custom css class by replacing default item css class with it, for this menu item if it exists.	  
		  oRow.className = spm_fixCSSForMac(this.getIntCSSName('spmitm') + (spm_getAttr(oRow, 'savecss', '') == '')?this.cssMenuItem:spm_getAttr(oRow, 'savecss', ''));
		  spm_getById("icon" + sID).className = spm_fixCSSForMac(this.getIntCSSName('spmicn') + this.cssMenuIcon);
		  spm_getById("td" + sID).className = spm_fixCSSForMac(this.getIntCSSName('spmitm') + ((spm_getAttr(oRow, 'savecss', '') == '')?this.cssMenuItem:spm_getAttr(oRow, 'savecss', '')));
		  //GG:End change
	...
	SolpartMenu.prototype.applyBorder = function (oCell, iSize, sTopLeftColor, sBottomRightColor, sStyle)
	{
		...
		if (this.moDisplay == 'HighLight')
		{
			if (sTopLeftColor == this.backColor)
			{
				...
				//GG:Line changed on 3/1/2004 by groskrg@versifit.com to support custom css class, by using the saved version, if exists, and not default item css class.
				oCell.className = spm_fixCSSForMac(this.getIntCSSName('spmbar spmitm') + ((spm_getAttr(oCell, 'savecss', '') == '')?this.cssMenuItem:spm_getAttr(oCell, 'savecss', '')));
...
//GG:Added on 7/28/2004 by groskrg@versifit.com to support custom highlight css class.					 
function spm_getMenuItemHLCSS(oNode)
{
  return spm_getAttr(oNode, "highlightcss", '');
}
//GG:End Add
...
//--- For JS DOM ---//
function SPJSXMLNode(sNodeName, sID, oParent, sTitle, sURL, sImage, sImagePath, sRightHTML, sLeftHTML, sRunAtServer, sItemStyle, sImageStyle, sToolTip, sItemCSS) 
{ 
