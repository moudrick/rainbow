		...
		{ 
			...
			{ 
				...
				MenuTreeNode mn = new MenuTreeNode(myTab.PageName);
					...
					// change the link to stay on the same page and call a category product
					mn.Link = HttpUrlBuilder.BuildUrl("~/DesktopDefault.aspx", tabIDItemsRoot, "ItemID=" + myTab.PageID); //fixed by manu
				mn.Width = Width; 
			...
			{ 
				...
				{ 
					...
					{ 
						...
						mnc.Link = HttpUrlBuilder.BuildUrl("~/DesktopDefault.aspx", tabIDItemsRoot, "ItemID=" + mySubTab.PageID); //by manu
						mnc.Width = mn.Width; 
