...
namespace Rainbow.DesktopModules 
{
	...
	public class Survey : PortalModuleControl 
	{
		...
		protected PlaceHolder SurveyHolder;
		...
		private void Page_Load(object sender, EventArgs e) 
		{
			...
			if ( Request.Cookies[CookieName] == null )
			{
                ...
                if ( RowCount > 0 )
				{
					...
					SubmitButton.Text = Localize.GetString("SURVEY_SUBMIT","Submit",this);
			...
			{	// Creation of Chart Survey - Begin
				...
				try
				{
					while (result.Read()) // Reads the list one-by-one
					{
						...
						lblOptDesc.Text = result["OptionDesc"].ToString() + "   " + perc.ToString("0") + "%  " + result["Num"].ToString() + " " + Localize.GetString("SURVEY_VOTES","votes",this)+"<br />";
		...
		{
			InitializeComponent();
			this.AddUrl = HttpUrlBuilder.BuildUrl("~/DesktopModules/Survey/SurveyEdit.aspx",this.PageID, "mID=" + ModuleID + "&dummy=0");
