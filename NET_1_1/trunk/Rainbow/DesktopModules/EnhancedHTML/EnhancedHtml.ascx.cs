	...
	public class EnhancedHtml : PortalModuleControl 
	{
		...
		private void Page_Load(object sender, EventArgs e) 
		{
			...
			if (modeID == "1") 
			{
				...
				string ehMultiPageMode = Localize.GetString("ENHANCEDHTML_MULTIPAGEMODE", "Multi Page Mode");
			...
			else 
			{
				...
				string ehPage = Localize.GetString("ENHANCEDHTML_PAGE", "Page");
				string ehOf = Localize.GetString("ENHANCEDHTML_OF", "of");
				string ehSinglePageMode = Localize.GetString("ENHANCEDHTML_SINGLEPAGEMODE", "Single Page Mode");
				...
				foreach (DataRow fila in paginas.Rows)
				{
					...
					{
						referencia = "<a href='" + dameUrl("&ModeID=0&EhPageID=" + i.ToString()) + "' " + "class='EnhancedHtmlLink'>" + fila["Title"] + "</a>";
						upmenu += referencia;
		...
		private DataTable giveMePages (bool addInvariantCulture)
		{
			...
			using (SqlDataReader dr = ehdb.GetLocalizedPages(ModuleID, portalSettings.PortalUILanguage.LCID, Version))
			{
				...
				{
					using (SqlDataReader dr2 = ehdb.GetLocalizedPages(ModuleID, CultureInfo.InvariantCulture.LCID, Version))
					{
						while (dr2.Read()) 
						{
		...
		private string dameUrl (string custom) 
		{
			...
			return HttpUrlBuilder.BuildUrl("~/" + HttpUrlBuilder.DefaultPage, PageID, ModuleID, null, custom, string.Empty, string.Empty);
		}
		...
		public EnhancedHtml()
		{
			...
			alignUpMenu.Add(new SettingOption(1, Localize.GetString("LEFT","Left")));
			alignUpMenu.Add(new SettingOption(2, Localize.GetString("CENTER","Center")));
			alignUpMenu.Add(new SettingOption(3, Localize.GetString("RIGHT","Right")));
			...
			alignDownMenu.Add(new SettingOption(1, Localize.GetString("LEFT","Left")));
			alignDownMenu.Add(new SettingOption(2, Localize.GetString("CENTER","Center")));
			alignDownMenu.Add(new SettingOption(3, Localize.GetString("RIGHT","Right")));
		...
		public override string SearchSqlSelect(int portalID, int userID, string searchString, string searchField)
		{
			...
			if (Version == WorkFlowVersion.Staging)
				dbTable += "_st";
			SearchDefinition s = new SearchDefinition(dbTable, "Title", "DesktopHtml", "CreatedByUser", "CreatedDate", searchField);
			...
			if (HttpContext.Current != null && HttpContext.Current.Items["PortalSettings"] != null)
			{
				...
				retorno += " AND ((itm.CultureCode = '" + pS.PortalUILanguage.LCID.ToString() + "') OR (itm.CultureCode = '" + CultureInfo.InvariantCulture.LCID.ToString() + "'))";
			}
			return retorno;
		}
		#endregion
