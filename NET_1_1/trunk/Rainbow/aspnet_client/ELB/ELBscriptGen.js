...
function ELB_init(ctlID, autopostback, selMode, childLists, parentList, swapTarget) {
	...
	if(swapTarget) {
		...
		//alert(ctlID + " selection(s): " + valTemp); // Debugging
		if(selMode == 1) {
			var sTemp;
			var iTemp;
...
function ELB_GetText(ctlID) {
	...
	if(tmpList.multiple) {
		var sVal;
		for(var i=0; i < tmpList.options.length; i++) {
			if(tmpList.options[i].selected) { sVal += "," + tmpList.options.text; }
			}
		tmpList = tmpList.substring(1, tmpList.length - 1);
		}
	else {
		return tmpList.options[tmpList.selectedIndex].text;
		}
	}
...
function ELB_AddItem(ctlID, aParams) {
	var oList = getObj(ctlID);
	oList.options.length += 1;
	var iNew = oList.options.length - 1;
	oList.options[iNew].value = aParams[0];
	oList.options[iNew].text = aParams[1];
	if(aParams[2]) { oList.options[iNew].text += aParams[2]; }
	if(aParams[3]) { oList.options[iNew].text += aParams[3]; }
	}
...
function ELB_HideList(ctlID) {
    var oList = getObj(ctlID);
    if(oList.style) {
		oList.style.visibility = "hidden";
		oList.style.visibility = "hide";
