...
namespace Rainbow.DesktopModules
{
	...
	public class PortalSearch : PortalModuleControl
	{
		...
		protected DropDownList ddSearchModule;
		...
		protected DropDownList ddSearchField;
		protected Label lblError;
		protected Literal lblModule;
		protected Literal lblTopic;
		protected DropDownList ddTopics;
		protected Literal lblField;
		...
		{	
			...
			if(!IsPostBack)
			{
				...
				ListItem tmpItem;
				tmpItem= new ListItem(Localize.GetString("PORTALSEARCH_ALL", "All", null), string.Empty);
				ddSearchField.Items.Add(tmpItem);
				tmpItem = new ListItem(Localize.GetString("PORTALSEARCH_TITLE", "Title", null), "Title");
				ddSearchField.Items.Add(tmpItem);
			}
		}
		...
		private void Search_Click(object sender, EventArgs e)
		{
			try 
			{
				...
				//All = no filter
				if	(topicName == Localize.GetString("PORTALSEARCH_ALL", "All", null))
					topicName = string.Empty;

				if(txtSearchString.Text.Length <= 2)
					throw new Exception(Localize.GetString("PORTALSEARCH_TONARROW", "Search string to narrow to be searched", null));
				...
				DataGrid1.DataBind();
				if (hits == 1)
					lblHits.Text = "1 " + Localize.GetString("PORTALSEARCH_HIT", "hit",null);
				else
					lblHits.Text = Convert.ToString(hits) + " " + Localize.GetString("PORTALSEARCH_HITS", "hit",null);
			}
			catch (Exception ex)
			{
		...
		private DataSet CreatePortalDS(DataSet ds)
		{
			...
			if (showModuleName) {
				...
				ds.Tables[0].Columns["Module"].Caption = Localize.GetString("PORTALSEARCH_MODULE", "Module",null);
			};
			if (showModuleTitle) {
				ds.Tables[0].Columns.Add("Module Title");
				ds.Tables[0].Columns["Module Title"].Caption = Localize.GetString("PORTALSEARCH_MODULETITLE", "Module Title",null);
			};
			if (showTitle) {
				ds.Tables[0].Columns.Add("Title");
				ds.Tables[0].Columns["Title"].Caption = Localize.GetString("PORTALSEARCH_TITLE", "Title",null);
				ds.Tables[0].Columns.Add("cleanTitle");
			};
			if (showAbstract) {
				ds.Tables[0].Columns.Add("Abstract");
				ds.Tables[0].Columns["Abstract"].Caption = Localize.GetString("ABSTRACT", "Abstract",null);
			};
			if (showCreatedByUser) {
				ds.Tables[0].Columns.Add("User");
				ds.Tables[0].Columns["User"].Caption = Localize.GetString("PORTALSEARCH_USER", "User",null);
			};
			if (showCreatedDate) {
				ds.Tables[0].Columns.Add("Date");
				ds.Tables[0].Columns["Date"].Caption = Localize.GetString("DATE", "Date",null);
			};
			if (showLink) {
				ds.Tables[0].Columns.Add("Link");
				ds.Tables[0].Columns["Link"].Caption = Localize.GetString("PORTALSEARCH_LINK", "Link",null);
			};
			if (showTabName) {
				ds.Tables[0].Columns.Add("Tab");
				ds.Tables[0].Columns["Tab"].Caption = Localize.GetString("PORTALSEARCH_TAB", "Tab",null);
			};
			if (showTestInfo) {
				ds.Tables[0].Columns.Add("TestInfo");
				ds.Tables[0].Columns["TestInfo"].Caption = Localize.GetString("PORTALSEARCH_TESTINFO", "TestInfo",null);
			};
			return ds;
		}
					...
					while( hits<=maxHits && portalSearchResult.Read() ) 
					{
						...
						switch (strModuleGuidID)
						{
							...
							case "EC24FABD-FB16-4978-8C81-1ADD39792377":  //Products
								// Manu
								int tabID = PortalSettings.GetRootPage(Convert.ToInt32(strTabID), portalSettings.DesktopPages).PageID;
						...
						if (showTitle)
						{
							...
							else
							{
								if (portalSearchResult.IsDBNull(1))
									strTmp = Localize.GetString("PORTALSEARCH_MISSING", "Missing",null);
								else
						...
						if (showCreatedByUser)
						{
							...
							if (strTmp == "unknown")
							{
								strTmp = Localize.GetString ( "UNKNOWN", "unknown");
							}
							dr["User"] = strTmp;
						}
