...
namespace Rainbow.Helpers
{
	...
	public class SearchHelper
	{
		...
		/// </returns>
		[Obsolete("Replace me")]
		public static SqlDataReader SearchPortal(int portalID, int userID, string searchModule, string searchString, string searchField, string sortField, string sortDirection, string topicName, string addExtraSQL)
		{
			...
			for (int i = 0; i <= modulesCount; i = i + 2)
			{
				...
				string assemblyName = Path.WebPathCombine(Path.ApplicationRoot.ToString(), "bin", arrModulesList[i]).ToString();
				...
				if (p != null)
				{
					...
					if (currentSearch != string.Empty && currentSearch != null)
			...
			using (SqlCommand sqlCommand = new SqlCommand(selectSQL, sqlConnection))
			{
				...
				{
					LogHelper.Logger.Log(LogLevel.Error, "Error in Search:SearchPortal()-> " + e.ToString() + " " + select.ToString(), e);
					throw new ApplicationException("Error in Search selection.");
				}
			}
		}
		...
		public static void AddToDropDownList(int portalID, ref DropDownList ddList)
		{
			using (SqlDataReader drModules = GetSearchableModules(portalID))
			{
				ddList.Items.Add(new ListItem(Localize.GetString("ALL", "All", null), string.Empty));
		...
		public static SqlDataReader GetSearchableModules(int portalID)
		{
			...
			using (SqlCommand myCommand = new SqlCommand("rb_GetSearchableModules", myConnection))
			{
		...
		public static string CreateTestSQL(ArrayList arrFields, string strSearchwords, bool useAnd)
		{
			...
			if (strSearchwords.IndexOf('"') > -1)
				SearchHelper.MarkPhrase(ref strSearchwords);
			...
			foreach (string strItem in arrWord)
			{
				...
				foreach (string strField in arrFields)
				{
					if (i > 0)
						strTmp = strTmp + " OR ";
					strTmp = strTmp + strField + " LIKE '%" + strWord + "%'";
					i = i + 1;
				}
				i = 0;
			}
		...
		public static string[] GetTopicList(int portalID)
		{
			...
			al.Add(Localize.GetString("PORTALSEARCH_ALL", "All", null));
