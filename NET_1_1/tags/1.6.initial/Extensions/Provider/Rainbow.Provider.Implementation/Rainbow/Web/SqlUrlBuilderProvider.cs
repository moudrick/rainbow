...
namespace Rainbow.Web
{
	...
	public class SqlUrlBuilderProvider : UrlBuilderProvider
	{
		...
		public override string BuildUrl(string targetPage, int pageID, int modID, CultureInfo culture, string customAttributes, string currentAlias, string urlKeywords)
		{
			...
			UrlBuilderHelper.GetUrlElements(pageID,_cacheMinutes,ref _isPlaceHolder, ref _tabLink, ref _urlKeywords, ref _pageName);
			
			//2_aug_2004 Cory Isakson
			//Begin Navigation Enhancements
			if (!(targetPage.ToLower().EndsWith(_ignoreTargetPage.ToLower()))) // Do not modify URLs when working with TabLayout Administration Page
			{
		...
		public override void Initialize(string name, NameValueCollection configValue)
		{
			...
			else
			{
				if(ConfigurationSettings.AppSettings["HandlerDefaultSplitter"] != null)
					_defaultSplitter = ConfigurationSettings.AppSettings["HandlerDefaultSplitter"];
			}
			...
			{
				if(ConfigurationSettings.AppSettings["HandlerFlag"] != null)
					_handlerFlag = ConfigurationSettings.AppSettings["HandlerFlag"];
			}
			...
			{
				if(ConfigurationSettings.AppSettings["LangInURL"] != null)
					_langInUrl = bool.Parse(ConfigurationSettings.AppSettings["LangInURL"]);
			}
		...
		{
			return bool.Parse(UrlBuilderHelper.PageSpecificProperty(pageID, UrlBuilderHelper.IsPlaceHolderID, _cacheMinutes).ToString());
		}
		...
		{
			return UrlBuilderHelper.PageSpecificProperty(pageID, UrlBuilderHelper.UrlKeywordsID,_cacheMinutes).ToString();
		}
		...
		{
			string _urlPageName = UrlBuilderHelper.PageSpecificProperty(pageID, UrlBuilderHelper.PageNameID, _cacheMinutes).ToString();
		...
		public override string DefaultPage
		{
			get
			{
				...
				string strTemp = this._friendlyPageName;
