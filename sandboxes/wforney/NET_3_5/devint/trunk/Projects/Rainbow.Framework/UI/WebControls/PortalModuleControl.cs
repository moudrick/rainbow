...
namespace Rainbow.Framework.Web.UI.WebControls
{
    ...
    public class PortalModuleControl : UserControl, ISearchable, IInstaller
    {
        ...
        private List<ModuleSetting> _moduleConfiguration = new List<ModuleSetting>();
        ...
        private List<ModuleSetting> _settings = new List<ModuleSetting>();
        ...
        /// </summary>
        [Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
        public List<ModuleSetting> BaseSettings
        {
...
*/
        ...
        public ModuleButton DownButton
        {
            get
            {
                if (downButton == null && HttpContext.Current != null)
                {
                    ...
                    if (CanArrows)
                    {
                        List<Module> sourceList = GetModules(ModuleConfiguration.PaneName.ToLower());
                        Module m = sourceList[sourceList.Count - 1];
        ...
        private void OrderModules(List<Module> list)
        {
            int i = 1;
        ...
        /// <returns></returns>
        private List<Module> GetModules(string pane)
        {
            // get the portal setting at the Tab level and not from this class as it is not refreshed

            var q = from m in ((Page)Page).portalSettings.ActivePage.Modules
                    where m.PageId == portalSettings.ActivePage.PageID && m.PaneName.ToLower() == pane.ToLower()
                    select m;
        ...
        private void RightLeft_Click(Object sender, EventArgs e)
        {
            ...
            List<Module> sourceList = GetModules(sourcePane);
            ...
            foreach (Module item in sourceList)
                admin.UpdateModuleOrder(item.moduleId, item.Order, sourcePane);

            // reorder the modules in the target pane
            List<Module> targetList = GetModules(targetPane);
            OrderModules(targetList);

            // resave the order
            foreach (Module item in targetList)
                admin.UpdateModuleOrder(item.moduleId, item.Order, targetPane);
        ...
        private void UpDown_Click(Object sender, EventArgs e)
        {
            ...
            List<Module> modules = GetModules(pane);
            ...
            ModulesDB admin = new ModulesDB();
            foreach (Module item in modules)
                admin.UpdateModuleOrder(item.moduleId, item.Order, pane);
        ...
        /// <returns></returns>
        public static List<ModuleSetting> GetModuleSettings(int moduleId)
        {
