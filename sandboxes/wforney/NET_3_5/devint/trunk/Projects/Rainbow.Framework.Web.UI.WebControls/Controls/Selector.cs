    ...
    public class Selector : LanguageSwitcher
    {
        ...
        protected override void OnChangeLanguage(LanguageSwitcherEventArgs e)
        {
            if (this.Context != null)
            {
                ...
                var customParams = this.Context.Request.QueryString.Keys.Cast<string>().Where(key => !key.ToLower().Equals("mid") && !key.ToLower().Equals("tabid") && !key.ToLower().Equals("lang")).Aggregate(string.Empty, (current, key) => current + ("&" + key + "=" + this.Context.Request.Params[key]));

                string returnUrl = HttpUrlBuilder.BuildUrl(
