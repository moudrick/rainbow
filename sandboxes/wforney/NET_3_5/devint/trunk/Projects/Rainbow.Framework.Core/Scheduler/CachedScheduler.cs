namespace Rainbow.Framework.Scheduler
{
    ...
    public class CachedScheduler : SimpleScheduler
    {
        ...
        {
            this.Cache = new SortedList(new TaskComparer(), cacheSize);
            this.FillCache();
        }
        ...
        public void FillCache()
        {
            using (var dr = this.LocalSchDb.GetOrderedTask())
            {
                ...
                while (dr.Read() && this.Cache.Count < this.Cache.Capacity)
                {
                    tsk = new SchedulerTask(dr);

                    lock (this.Cache.SyncRoot)
                    {
                        this.Cache.Add(tsk, tsk);
                    }
                }
        ...
        public override SchedulerTask InsertTask(SchedulerTask task)
        {
            ...
            if (this.Cache.Count != 0 && task.DueTime < ((SchedulerTask)this.Cache.GetKey(this.Cache.Count - 1)).DueTime)
            {
                lock (this.Cache.SyncRoot)
                {
                    this.Cache.RemoveAt(this.Cache.Count - 1);
                    this.Cache.Add(task, task);
                }
            }
        ...
        protected override void Schedule(object timerState)
        {
            lock (this)
            {
                ...
                {
                    var task = (SchedulerTask)this.Cache.GetKey(0);
        ...
        private class TaskComparer : IComparer
        {
            ...
            public int Compare(object x, object y)
            {
                ...
                {
                    throw new ArgumentNullException("x", "object is not a SchedulerTask");
                }
